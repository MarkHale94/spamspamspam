...
let customers = {
  ...
  // From that object extract just the customers' email addresses and store them in a new array. You will need to use methods such as map(), forEach() and/or concat() to accomplish this.
  let customerDataArray = customers.customers;
// the index argument is the current index of the customerDataArray. This returns the information for each index in the array.

const emailArray = customerDataArray.map((index)=>{
return index.contacts.email;
});

// const newEmailArray = emailArray[0].concat(emailArray[1],emailArray[2],emailArray[3],emailArray[4],emailArray[5],emailArray[6],emailArray[7],emailArray[8],emailArray[9]);
console.log(emailArray);
// console.log(newEmailArray);

let bestEmailArray = [];
const emailMerge = emailArray.map((index) => {
  console.log(index);
  bestEmailArray = bestEmailArray.concat(index);
});
console.log(bestEmailArray);